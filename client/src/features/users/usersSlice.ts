...
import { IEvent, IStore, IUsersInitialState } from "../../declarations/index";
...
const usersSlice = createSlice({
  ...
  initialState,
  reducers: {
    createdEvent: (state, action) => {
      state.userPersonalProfile?.createdEvents.push(action.payload);
    },
    subscribedToEvent: (state, action) => {
      state.userPersonalProfile?.joinedEvents.push(action.payload);
    },
    unsubscribedFromEvent: (state, action) => {
      if (state.userPersonalProfile?.joinedEvents.length) {
        state.userPersonalProfile.joinedEvents = state.userPersonalProfile.joinedEvents.filter(
          (event: IEvent) => event._id !== action.payload
        );
      }
    },
    eventsSelected: (state, action) => {
      state.eventsSelection = action.payload;
    },
  },
...
export const selectPersonalProfileEvents = (state: IStore) =>
  state.users.eventsSelection;
